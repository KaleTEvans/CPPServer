...
namespace TwsApi
{
	...
	ENUMValues( GenericTicks )                   // resulting tick values, see TickType
		...
		ENUM_V( AverageOptionVolume      , 105 ) // Average volume of the corresponding option contracts, 87
	...
	//	ENUM_V( MarkPrice                , 232 ) // ?via error msg?
		...
		ENUM_V( Shortable                , 236 ) //	, 89
		ENUM_V( Inventory                , 256 ) //	,  
		ENUM_V( FundamentalRatios        , 258 ) //	, 47
		ENUM_V( News                     , 292 ) // Contracts news feed, 62
		ENUM_V( TradeCount               , 293 ) // Trade count for the day, 54
		ENUM_V( TradeRate                , 294 ) // Trade count per minute, 55
		ENUM_V( VolumeRate               , 295 ) // Volume per minute, 56
		ENUM_V( LastRTHTrade             , 318 ) // Last regular hours traded price, 57 
		ENUM_V( RTTradeVolume            , 375 ) // Last trade details that excludes "unreportable trades", 77
		ENUM_V( RTHistoricalVolatility   , 411 ) //
		ENUM_V( IBDividends              , 456 ) //
		ENUM_V( FuturesOpenInterest      , 588 ) // 86
	...
	ENUMValues( TickTypes )                 // Used in these EWrapper calls , Value of genericTicks
		...
		ENUM_N( RTTradeVolume           )   // 77 tickString()
		ENUM_N( CreditmanMarkPrice      )   // 78 tickPrice()
		ENUM_N( CreditmanSlowMarkPrice  )   // 79 tickPrice()
		ENUM_N( DelayedBidOption        )   // 80 tickPrice()
		ENUM_N( DelayedAskOption        )   // 81 tickPrice()
		ENUM_N( DelayedLastOption       )   // 82 tickPrice()
		ENUM_N( DelayedModelOption      )   // 83 tickPrice()
		ENUM_N( LastExchange            )   // 84 tickString()
		ENUM_N( LastRegulatoryTime      )   // 85 tickString()
		ENUM_N( FuturesOpenInterest     )   // 86 tickSize()
		ENUM_N( AverageOptionVolume     )   // 87 tickSize()
		ENUM_N( NotSet                  )	//
